 ...
 */
typedef void (*sn_failedMethodCallback)(Class, Method);

/**
 * Used with #sn_injectMethods to determine injection behavior.
 */
typedef NS_OPTIONS(NSUInteger, sn_methodInjectionBehavior) {
    /**
     * Indicates that any existing methods on the destination class should be
     * overwritten.
     */
    sn_methodInjectionReplace                  = 0x00,

    /**
     * Avoid overwriting methods on the immediate destination class.
     */
    sn_methodInjectionFailOnExisting           = 0x01,
    
    /**
     * Avoid overriding methods implemented in any superclass of the destination
     * class.
     */
    sn_methodInjectionFailOnSuperclassExisting = 0x02,

    /**
     * Avoid overwriting methods implemented in the immediate destination class
     * or any superclass. This is equivalent to
     * <tt>sn_methodInjectionFailOnExisting | sn_methodInjectionFailOnSuperclassExisting</tt>.
     */
    sn_methodInjectionFailOnAnyExisting        = 0x03,

    /**
     * Ignore the \c +load class method. This does not affect instance method
     * injection.
     */
    sn_methodInjectionIgnoreLoad = 1U << 2,

    /**
     * Ignore the \c +initialize class method. This does not affect instance method
     * injection.
     */
    sn_methodInjectionIgnoreInitialize = 1U << 3
};

/**
 * A mask for the overwriting behavior flags of #sn_methodInjectionBehavior.
 */
static const sn_methodInjectionBehavior sn_methodInjectionOverwriteBehaviorMask = 0x3;
...
typedef enum {
     ...
     */
    sn_propertyMemoryManagementPolicyAssign = 0,

    /**
     * The value is retained.
     */
    sn_propertyMemoryManagementPolicyRetain,

    /**
     * The value is copied.
     */
    sn_propertyMemoryManagementPolicyCopy
} sn_propertyMemoryManagementPolicy;
...
typedef struct {
     ...
     * #sn_propertyMemoryManagementPolicyAssign if #readonly is \c YES.
     */
    sn_propertyMemoryManagementPolicy memoryManagementPolicy;
 ...
 */
Class *sn_copySubclassList (Class aClass, unsigned *subclassCount);
 ...
 */
#define sn_getIvar(OBJ, IVAR, TYPE) \
 ...
 * or floating-point types.
 */
#define sn_getIvarByName(OBJ, NAME, TYPE) \
    sn_getIvar((OBJ), class_getInstanceVariable(object_getClass((OBJ)), (NAME)), TYPE)
 ...
 */
unsigned sn_injectMethods (Class aClass, Method *methods, unsigned count, sn_methodInjectionBehavior behavior, sn_failedMethodCallback failedToAddCallback);

/**
 * Invokes #sn_injectMethods with the instance methods and class methods from
 * \a srcClass. #sn_methodInjectionIgnoreLoad is added to #behavior for class
 * method injection.
 *
 * Returns whether all methods were added successfully. For each method that fails
 * to be added, \a failedToAddCallback (if provided) is invoked.
 ...
 */
BOOL sn_injectMethodsFromClass (Class srcClass, Class dstClass, sn_methodInjectionBehavior behavior, sn_failedMethodCallback failedToAddCallback);
 ...
 *
 * @li Each protocol is loaded with #sn_loadSpecialProtocol and a custom block
 ...
 * #sn_specialProtocolReadyForInjection.
 ...
 * #sn_specialProtocolReadyForInjection as well.
 *
