...
#define concreteprotocol(NAME) \
    ...
    + (void)load { \
         ...
         * the second
         */ \
        if (!sn_addConcreteProtocol(objc_getProtocol(metamacro_stringify(NAME)), self)) \
    ...
    static void sn_ ## NAME ## _inject (void) { \
        /*
         * use this injection point to mark this concrete protocol as ready for
         * loading
         */ \
        sn_loadConcreteProtocol(objc_getProtocol(metamacro_stringify(NAME))); \
    }

/*** implementation details follow ***/
BOOL sn_addConcreteProtocol (Protocol *protocol, Class methodContainer);
