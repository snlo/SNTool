...
typedef void (^sn_specialProtocolInjectionBlock)(Class);

// a `const char *` equivalent to system struct objc_method_description
typedef struct {
    SEL name;
    const char *types;
} sn_methodDescription;
 ...
 */
static void sn_injectSpecialProtocols (void) {
    /*
	...
	// use this instead of sn_copyClassList() to avoid sending +initialize to
    ...
    @autoreleasepool {
        ...
        for (size_t i = 0;i < specialProtocolCount;++i) {
            ...
            // from the structure
            sn_specialProtocolInjectionBlock injectionBlock = (__bridge_transfer id)specialProtocols[i].injectionBlock;
...
unsigned sn_injectMethods (
    Class aClass,
    Method *methods,
    unsigned count,
    sn_methodInjectionBehavior behavior,
    sn_failedMethodCallback failedToAddCallback
) {
    ...
    @autoreleasepool {
        ...
        if (!isMeta) {
            ...
            behavior &= ~(sn_methodInjectionIgnoreLoad | sn_methodInjectionIgnoreInitialize);
        }
        ...
        for (unsigned methodIndex = 0;methodIndex < count;++methodIndex) {
            ...
            if (behavior & sn_methodInjectionIgnoreLoad) {
            ...
            if (behavior & sn_methodInjectionIgnoreInitialize) {
            ...
            switch (behavior & sn_methodInjectionOverwriteBehaviorMask) {
            case sn_methodInjectionFailOnExisting:
                success = class_addMethod(aClass, methodName, impl, type);
                break;

            case sn_methodInjectionFailOnAnyExisting:
            ...
            case sn_methodInjectionReplace:
                class_replaceMethod(aClass, methodName, impl, type);
                break;

            case sn_methodInjectionFailOnSuperclassExisting:
                {
...
BOOL sn_injectMethodsFromClass (
    Class srcClass,
    Class dstClass,
    sn_methodInjectionBehavior behavior,
    sn_failedMethodCallback failedToAddCallback)
{
    ...
    // ignore +load
    behavior |= sn_methodInjectionIgnoreLoad;
    addedCount = sn_injectMethods(
...
Class sn_classBeforeSuperclass (Class receiver, Class superclass) {
...
Class *sn_copyClassList (unsigned *count) {
...
unsigned sn_addMethods (Class aClass, Method *methods, unsigned count, BOOL checkSuperclasses, sn_failedMethodCallback failedToAddCallback) {
    sn_methodInjectionBehavior behavior = sn_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= sn_methodInjectionFailOnSuperclassExisting;

    return sn_injectMethods(
        aClass,
        methods,
        count,
        behavior,
...
BOOL sn_addMethodsFromClass (Class srcClass, Class dstClass, BOOL checkSuperclasses, sn_failedMethodCallback failedToAddCallback) {
    sn_methodInjectionBehavior behavior = sn_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= sn_methodInjectionFailOnSuperclassExisting;
    
    return sn_injectMethodsFromClass(srcClass, dstClass, behavior, failedToAddCallback);
}

BOOL sn_classIsKindOfClass (Class receiver, Class aClass) {
    while (receiver) {
        if (receiver == aClass)
            return YES;
...
Class *sn_copyClassListConformingToProtocol (Protocol *protocol, unsigned *count) {
    Class *allClasses;
    ...
    @autoreleasepool {
        ...
        allClasses = sn_copyClassList(&classCount);
...
sn_propertyAttributes *sn_copyPropertyAttributes (objc_property_t property) {
    ...
    sn_propertyAttributes *attributes = calloc(1, sizeof(sn_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate sn_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = sn_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = sn_propertyMemoryManagementPolicyRetain;
            break;
...
Class *sn_copySubclassList (Class targetClass, unsigned *subclassCount) {
    unsigned classCount = 0;
    Class *allClasses = sn_copyClassList(&classCount);
...
Method sn_getImmediateInstanceMethod (Class aClass, SEL aSelector) {
...
BOOL sn_getPropertyAccessorsForClass (objc_property_t property, Class aClass, Method *getter, Method *setter) {
    sn_propertyAttributes *attributes = sn_copyPropertyAttributes(property);
    if (!attributes)
        return NO;
...
NSMethodSignature *sn_globalMethodSignatureForSelector (SEL aSelector) {
    ...
    static sn_methodDescription volatile methodDescriptionCache[selectorCacheLength];
    ...
    sn_methodDescription methodDesc;
    ...
    methodDesc = (sn_methodDescription){.name = NULL, .types = NULL};

    uint classCount = 0;
    Class *classes = sn_copyClassList(&classCount);
    ...
    if (classes) {
        @autoreleasepool {
            ...
            for (uint i = 0;i < classCount;++i) {
                ...
                if (method) {
                    methodDesc = (sn_methodDescription){.name = aSelector, .types = method_getTypeEncoding(method)};
                    break;
                }
            }
        }
    ...
    if (!methodDesc.name) {
        ...
        if (protocols) {
            ...
            for (uint i = 0;i < protocolCount;++i) {
                ...
                if (objcMethodDesc.name) {
                    methodDesc = (sn_methodDescription){.name = objcMethodDesc.name, .types = objcMethodDesc.types};
                    break;
                }
            }
            free(protocols);
...
BOOL sn_loadSpecialProtocol (Protocol *protocol, void (^injectionBehavior)(Class destinationClass)) {
    @autoreleasepool {
        ...
        // sn_injectSpecialProtocols()
        #ifndef __clang_analyzer__
        sn_specialProtocolInjectionBlock copiedBlock = [injectionBehavior copy];
...
void sn_specialProtocolReadyForInjection (Protocol *protocol) {
    @autoreleasepool {
        NSCParameterAssert(protocol != nil);
...
void sn_removeMethod (Class aClass, SEL methodName) {
    Method existingMethod = sn_getImmediateInstanceMethod(aClass, methodName);
    if (!existingMethod) {
        return;
    }
...
void sn_replaceMethods (Class aClass, Method *methods, unsigned count) {
    sn_injectMethods(
        aClass,
        methods,
        count,
        sn_methodInjectionReplace,
        NULL
    );
}

void sn_replaceMethodsFromClass (Class srcClass, Class dstClass) {
    sn_injectMethodsFromClass(srcClass, dstClass, sn_methodInjectionReplace, NULL);
}

NSString *sn_stringFromTypedBytes (const void *bytes, const char *encoding) {
    switch (*encoding) {
